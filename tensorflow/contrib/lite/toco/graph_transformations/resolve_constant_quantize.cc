...
bool ResolveConstantQuantize::Run(Model* model, std::size_t op_index) {
  const auto quantize_it = model->operators.begin() + op_index;
  auto* quantize_base_op = quantize_it->get();
  if (quantize_base_op->type != OperatorType::kQuantize) {
    return false;
  }

  auto* quantize_op =
      static_cast<QuantizeOperator*>(quantize_base_op);

  if (quantize_op->inputs.size() == 1) {
    return false;
  }

  CHECK(quantize_op->inputs.size() == 3);
  // This transformation only applies when the input array is constant.
  if (!IsConstantParameterArray(*model, quantize_op->inputs[1]) ||
      !IsConstantParameterArray(*model, quantize_op->inputs[2])) {
    return false;
  }

  auto& input_array = model->GetArray(quantize_op->inputs[0]);
  auto& output_array = model->GetArray(quantize_op->outputs[0]);
  CHECK(input_array.data_type == ArrayDataType::kFloat);
  output_array.data_type = ArrayDataType::kUint8;
  CHECK(!output_array.buffer);

  // inputs[1] is min, inputs[2] is max
  const auto& input1_array = model->GetArray(quantize_op->inputs[1]);
  const auto& input2_array = model->GetArray(quantize_op->inputs[2]);
